...
class ForceClient(Node):
    ...
    def PID_control(self, target_state):
        ...
        # 平均误差计算（绝对值平均）
        average_error = sum(abs(e) for e in error) / len(error)
        # 储存100个历史平均误差用于计算平均误差的方差
        if not hasattr(self, '_average_errors'):
            self._average_errors = []
        self._average_errors.append(average_error)
        if len(self._average_errors) > 100:
            self._average_errors.pop(0)

        # 计算历史平均误差的方差（如果历史数据足够）
        if len(self._average_errors) > 1:
            mean_avg_error = sum(self._average_errors) / len(self._average_errors)
            variance_avg_error = sum((e - mean_avg_error) ** 2 for e in self._average_errors) / (len(self._average_errors) - 1)
            self.get_logger().info(f"平均误差: {average_error:.3f}, 平均误差方差: {variance_avg_error:.6f}")
            
            # 保存此时的输入和状态到文件中，用于参数辨识
            if variance_avg_error <= 0.000001:
                with open('pid_data_log.txt', 'a') as f:
                    f.write(f"{self.state} ,{self.force_values}\n")
        else:
            # 历史数据不足，跳过记录
            pass
